...
set :application, 'Shinetech Demo'
set :repo_url, ''
...
# rvm
set :rvm_ruby_version, "2.3.0@#{fetch(:application)}_#{fetch(:stage)}"
...
task import_index: :environment do
  queue "cd #{app_path}"
  ['Doctor'].each do |c|
    queue "RAILS_ENV=#{fetch(:stage)} ./bin/rake elasticsearch:import CLASS=\"#{c}\" FORCE=true"
  end
end
